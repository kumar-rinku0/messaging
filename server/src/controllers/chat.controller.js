...
const handleGetChatById = async (req, res) => {
  ...
  const updatedChat = {
    ...
    displayName:
      chat.type === "group"
        ? chat.name
        : members.find((m) => m._id.toString() !== userId)?.username,
    displayAvatar:
      chat.type === "group"
        ? chat.avatar
        : members.find((m) => m._id.toString() !== userId)?.avatar,
  };
...
const handleGetPrivateChats = async (req, res) => {
...
const handleGetGroupChats = async (req, res) => {
  const { userId } = req.params;
  const { sort } = req.query;
  const query = {
    members: { $all: [userId] },
...
const handleGetAllTypeChats = async (req, res) => {
  const { userId } = req.params;
  // const { sort } = req.query;
  // const query = {
  //   members: { $all: [userId] },
...
const handleUpdateChatById = async (req, res) => {
  const { chatId } = req.params;
